  ...
  useEffect(() => {
    createClient()
      ...
      .then(({ data, error }) => {
        ...
        setAssets(!error && data ? (data as MarketplaceRow[]).map((row) => ({
          id: row.listing_code,
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-6 flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as AssetStatus | "all")} className="bg-white border border-border rounded-xl px-3 py-2 text-sm text-gray-700 focus:outline-none">
            ...
            {selectedAsset && (
              <div className="w-80 bg-white border border-border rounded-2xl p-5 flex flex-col gap-4 shrink-0 overflow-y-auto max-h-[70vh]">
                ...
                <div className="flex flex-col gap-2 pt-2 border-t border-border">
                  <Link href="/manage/scan" className="flex items-center justify-center gap-2 py-2 rounded-xl bg-saffron-500 text-white text-xs font-bold hover:bg-saffron-600">
                    <ScanLine size={13} /> Scan QR
                  </Link>
                  <Link href="/manage/redistribution" className="flex items-center justify-center gap-2 py-2 rounded-xl border border-border text-xs font-bold text-gray-700 hover:bg-surface">
