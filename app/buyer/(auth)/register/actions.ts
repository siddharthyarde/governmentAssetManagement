...
export async function registerInstitutionAction(formData: FormData) {
  ...
  if (!parsed.success) {
    ...
    redirect(`/buyer/register?error=${encodeURIComponent(firstError.message)}`);
  }
  ...
  if (signUpError) {
    redirect(`/buyer/register?error=${encodeURIComponent(signUpError.message)}`);
  }

  if (!authData.user) {
    redirect("/buyer/register?error=Account+creation+failed");
  }
  ...
  if (instError) {
    redirect(`/buyer/register?error=${encodeURIComponent(instError.message)}`);
  }
  ...
  redirect("/buyer/dashboard?registered=1");
}
