...
export default function BuyerResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/buyer/auth/callback?next=/buyer/auth/update-password`,
    });
    if (err) {
          ...
          <>
            <div className="mb-6">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Enter your registered official email and we&apos;ll send you a reset link.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full rounded-lg bg-secondary py-2.5 text-sm font-semibold text-white
                           hover:bg-secondary/90 active:scale-[0.98] transition-all disabled:opacity-60"
              >
            ...
            <div className="mt-5 pt-5 border-t border-border text-center">
              <Link href="/buyer/login" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
                <ArrowLeft size={14} /> Back to Sign In
              </Link>
            </div>
          </>
