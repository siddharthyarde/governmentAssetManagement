...
export default async function BuyerLoginPage({ searchParams }: Props) {
  const { error, next = "/buyer/dashboard" } = await searchParams;
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm p-8">
        ...
        <form action={loginAction} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between mb-1.5">
              ...
              <a
                href="/buyer/auth/reset-password"
        ...
        <div className="mt-5 pt-5 border-t border-border space-y-3">
          ...
          <p className="text-center text-sm text-muted-foreground">
            New institution?{" "}
            <a href="/buyer/register" className="text-secondary font-semibold hover:underline">
              Register now →
            </a>
          </p>
        </div>
